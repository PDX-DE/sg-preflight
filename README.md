...
python -m sg_preflight api-version-coverage --repo-root C:\repositories\trunk --json
python -m sg_preflight country-variant-coverage --repo-root C:\repositories\trunk --json
python -m sg_preflight export-size-trend --repo-root C:\repositories\trunk --json
...
The curated bundle test count can differ from the source test count because internal guard-only tests are excluded from the team-facing bundle. The most recent full source verification for this tree ran 1,188 tests OK with 8 skipped (exclusive `unittest discover` run, 2026-07-20).
