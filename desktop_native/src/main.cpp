...
using sg_preflight::native_shell::ShellScreen;
using sg_preflight::native_shell::ShellLanguage;
using sg_preflight::native_shell::UiText;
using sg_preflight::native_shell::BuildHelpPromptMessage;
using sg_preflight::native_shell::FriendlyActionDescription;
using sg_preflight::native_shell::ShortActionLabel;
...
void RenderButtonGuide(ShellState& state) {
    ...
    const auto activate_item = [&](const GuideItem& item) {
        if (std::strcmp(item.id, "guide-prompt-next") == 0) {
            ...
            OpenPrompt(state, Tr(state, UiText::Help), BuildHelpPromptMessage(state.current_screen), false, false, false);
        }
    };
