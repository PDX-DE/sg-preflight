...
class TestDesktopEvidenceModel(unittest.TestCase):
    def test_clean_compatibility_host_remains_isolated_from_the_qt_quick_dispatcher(self) -> None:
        ...
        self.assertNotIn("switch_requested", host_source)
        self.assertNotIn("grafiks_button", host_source)
        self.assertNotIn('QPushButton("Grafiks"', host_source)
        self.assertNotIn("grafiksWipNotice", theme_source)
    ...
    def test_clean_host_embeds_nicegui_without_external_browser(self) -> None:
        ...
        self.assertIn("DASHBOARD_STARTUP_TIMEOUT_SECONDS = 120", host_source)
        self.assertLess(host_source.index("self._start_server()"), host_source.index("QWebEngineView"))
        self.assertIn("first launch can take up to a minute", host_source)
        self.assertIn("_stop_server", host_source)
        self.assertIn("except Exception:", host_source)
        self.assertLess(host_source.index("except Exception:"), host_source.index("def _server_command"))
        self.assertIn("self.status_label.hide()", host_source)
        self.assertIn("controller.close_all()", app_source)
        self.assertNotIn("switch_requested", host_source)
        self.assertNotIn("Grafiks", host_source)
