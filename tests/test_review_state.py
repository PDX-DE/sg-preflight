...
class TestReviewState(unittest.TestCase):
    ...
    def test_build_review_board_state_merges_package_and_latest_snapshot(self) -> None:
        ...
        self.assertIn("follow_up", state["review_owner_decisions"]["status_options"])
        ...
        self.assertEqual(state["external_findings"]["headline"], "1/1 reported")
        ...
        self.assertEqual(state["top_review_priority_items"][0]["attention_category"], "must inspect")
        self.assertIn("technical blocker", state["top_review_priority_items"][0]["signals"])
        self.assertTrue(state["daily_delta_summary"]["operator_signal"])
        self.assertEqual(state["daily_delta_summary"]["review_first_preview"][0], "NA8: `default` generated a candidate output; baseline review can be done quickly.")
        self.assertNotIn("approved", state["daily_delta_summary"]["review_first_preview"][0].lower())
        self.assertNotIn("validated", state["daily_delta_summary"]["review_first_preview"][0].lower())
        self.assertTrue(state["operator_next_step"])
    ...
    def test_load_helpers_return_structured_json_sources(self) -> None:
        ...
        self.assertEqual(priority["top_items"][0]["attention_category"], "must inspect")
        self.assertIn("technical blocker", priority["top_items"][0]["signals"])
        self.assertEqual(delta["top_five_to_review"][0], "NA8: `default` generated a candidate output; baseline review can be done quickly.")
        self.assertNotIn("approved", delta["top_five_to_review"][0].lower())
        self.assertNotIn("validated", delta["top_five_to_review"][0].lower())
