...
class TestCLI(unittest.TestCase):
    ...
    def test_rack_readiness_cli_writes_json_and_markdown(self) -> None:
        ...
        self.assertEqual(payload["rack_inventory"]["racks"][0]["id"], "ITAI-1906")
        self.assertFalse(payload["rack_inventory"]["live"])
        self.assertFalse(payload["rack_inventory"]["verified_by_sgfx"])
        self.assertIn("not live", payload["rack_inventory"]["provenance_note"].casefold())
        self.assertIn("sgfx does not measure or verify", payload["rack_inventory"]["provenance_note"].casefold())
        self.assertIn("Test-app Launch->Rendered", {metric["name"] for metric in payload["kpi_reference"]["metrics"]})
        self.assertFalse(payload["kpi_reference"]["live"])
        self.assertFalse(payload["kpi_reference"]["measured_by_sgfx"])
        self.assertIn("not live", payload["kpi_reference"]["provenance_note"].casefold())
        self.assertIn("sgfx does not measure or verify", payload["kpi_reference"]["provenance_note"].casefold())
        self.assertTrue(json_exists)
        self.assertTrue(markdown_exists)
        self.assertFalse(payload["is_approval"])
    ...
    def test_rack_readiness_cli_text_marks_svt_as_reference_only(self) -> None:
        ...
        self.assertIn("rack target inventory reference", output)
        self.assertIn("kpi expectation reference", output)
        self.assertIn("reference - not live", output)
        self.assertIn("itai-1906", output)
        self.assertIn("compose->rendered", output)
        self.assertIn("not measured by sgfx", output)
