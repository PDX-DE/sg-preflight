...
- working tree: the native shell now defaults to a readable `WORK` display mode with lighter scanlines/static, lower prompt shadows, calmer selected-card contrast, sharper readable body/small fonts, a wider work-screen canvas, a larger run-log focus, copyable file/report paths, and no background music unless the operator explicitly switches back to `CINEMATIC`
