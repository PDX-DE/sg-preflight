...
- working tree: the native `ENV` page now truly renders on the main shell path instead of advancing into a blank screen, and the doctor page now shows real readiness content in verifier captures while the shell navigation/render switch no longer skips `ShellScreen::Environment`
- working tree: Environment Doctor and SG checker readiness now distinguish `tool exists` from `tool can actually open SG scenes here`, so `RaCoHeadless` / `Ramses Composer` report a truthful partial/incompatible state when the configured build rejects the representative `.rca` with `Project feature level 4 outside valid range (1 ... 2)`, and scene-check actions now block honestly instead of pretending they are runnable
- working tree: shell audio now prefers workspace `SERGFX.wav` / `BAChefPeePee.wav` before MP3, bundle staging can carry those WAV music assets when explicitly requested, and the latest native verifier trace confirms the shell now resolves `UI music_track=SERGFX.wav`
- working tree: popup/help text now follows the real Unleashed message-window path more closely with a lighter single-shadow treatment and slightly smaller informational banner copy, reducing the heavier fake-outline look while staying inside the same Seurat/installer presentation
- working tree: `docs/bmw-access-integration-checklist.md` now captures the blocked BMW-side intake path explicitly, so repo access, smoke-script discovery, target mapping, first dry run, and first evidence-bundle steps are documented before BMW access arrives
- working tree: the native shell now stays on the heavier SERGFX / Unleashed-style presentation by default again, restoring the full animated green scanline/checkerboard chrome and stylized font treatment while keeping the newer spacing/readability corrections inside that same cinematic shell instead of splitting the product into `WORK` vs `CINEMATIC`
- working tree: the native shell now owns a single executable-local `imgui.ini` for both ImGui window state and SERGFX shell settings, so `display_mode`, `music_enabled`, and `sfx_enabled` persist without being wiped by Dear ImGui's default save path, and plain native builds are now seeded with that same cinematic/music-on default file after every build
- working tree: prompt/message-window body text now uses a tighter Unleashed-style Seurat shadow treatment again instead of the flatter readability-only pass, restoring the thicker installer/message-window font feel while shrinking the oversized informational popup text back down
- working tree: the native wizard now adds a real `ENV` / Environment Doctor page backed by `python -m sg_preflight desktop-state environment`, so the shell can surface Python/backend readiness, mirrored SG checker assets, delivery-checklist coverage, RaCoHeadless, Blender configuration, BMW blockers, Jira/QA Hero blocked status, and output write access from the same shared backend contract as the rest of the desktop state
- working tree: the native verifier now accepts `-ProfileId` / `-ActionId`, can start the shell on a chosen slice/check, and now captures deeper proof screenshots for `FILES`, `ENV`, and `STAGES` once a fast action finishes, while the backend trace proves the new environment-doctor call and the full `RUN -> FILES -> ENV -> STAGES` flow
- working tree: the native shell now supports manual evidence attachments into the active action bundle, first-pass `OPEN FOLDER` / `OPEN PROJECT ROOT` / `OPEN IN BLENDER` / `OPEN IN RACO` adapters on evidence and artifact paths, queued native screenshot attachment, and clearer failed-action diagnostics with copy/reveal/retry actions on the `RUN` screen
- working tree: the Ramses Composer GUI adapter now launches `.rca` scenes with `--project "<path>"` instead of passing the raw path alone, matching the real Composer CLI contract so the shell opens projects directly instead of dumping the usage window
...
- working tree: the shell header now stays on the workspace `debug_icon` in-app while the Windows executable resource keeps `exe_ico`, and active `Run` states switch to a cleaner live-console layout that gives the signal log more room while deferring history clutter until the check settles
- working tree: background music now prefers workspace `SERGFX.mp3`, respects the existing `imgui.ini` `music_enabled` toggle, and can randomly swap to `BAChefPeePee.mp3` as a boot-time easter egg while the WAV-based installer SFX path stays unchanged
- working tree: native builds now regenerate `desktop_native/resources/exe_ico.ico` from the current root `exe_ico.png` before compiling, and portable bundle staging now seeds the bundle-root `imgui.ini` with the intended shell defaults instead of mutating a separate `workspace\imgui.ini`
- working tree: the native C++ shell now swaps the OG header badge over to the workspace `debug_icon`, wraps it in the real `arrow_circle` / `pulse_install` motion on every screen, replaces the patchy header/footer static overlays with a cleaner full-width scanline/grid treatment, and drives the left installer art from screen-aware `framework_icon` / `game_icon` motion instead of the old generic crossfade
- working tree: the footer guide now keeps primary actions on the real key-style prompts instead of flipping to a mouse icon in mouse mode, uses the full `kb_key_F1` / `F2` / `F3` / `F4` art at the same size as `Esc`, and embeds the new `exe_ico` resource into the Windows executable while the portable bundle now carries `debug_icon.png` / `exe_ico.png`
- working tree: SERGFX intro/help copy now explains `Project HMI & SDET` in clearer automotive-visualisation / test-engineering language, live `Run` refreshes poll a little slower, and the active log panel trims to the latest tail while a check is still running so the native shell stops repainting the full growing log every frame during long verifier sessions
- working tree: the native C++ shell now rebrands its operator-facing chrome around `SERGFX` / `SERGFX CHECKING`, ships `v1.0.0`, loads the new workspace `framework_icon` / `game_icon` art into the animated left installer slot, and embeds `debug_icon.ico` into the native Windows executable resource
- working tree: the Select page now uses a tighter installer-style layout with the full six-check grid visible, a wider available-slices rail, and cleaner footer/help sizing, while `Run` now exposes `F2` raw-log and `F3` report shortcuts through the same real key art used in the footer guide
- working tree: active-run polling is now slower and lighter on the Python/backend side (`Run` every 5s, follow-up pages every 6s, linked result refreshes every 8th token), and the DX12 screenshot path now writes through a committed temp PNG swap so long verifier runs stop producing corrupted capture files
- working tree: the native C++ shell now keeps active-run polling strictly on run-oriented pages instead of waking the Python/backend side while operators are still on intro/select/review, the installer footer uses the cropped real `F1` key art plus the fixed `v1.0.0` version label from the OG installer path, and the verification harness now proves that behavior over a full 10-minute observed run with fresh screenshots and a clean automated quit
...
- working tree: the native C++ shell now lets the OG quit prompt banner fall behind a second dark overlay once the `Yes`/`No` chooser opens, shifts full-screen blackout into the late quit window so the installer chrome disappears in stages first, shortens the Select-page headline, and removes the duplicated `Selected Mode` text block so the slice/action screen gets back more vertical room for the actual action grid
