...
if ([string]::IsNullOrWhiteSpace($EvidenceRoot)) {
    ...
    $EvidenceRoot = Join-Path $repoRoot "out\cpp\ramses-rmlui-planet-hub-h2-$stamp"
}

New-Item -ItemType Directory -Force -Path $EvidenceRoot | Out-Null
$transcriptPath = Join-Path $EvidenceRoot "ramses-rmlui-planet-hub-h2-transcript.txt"
$outputPath = Join-Path $EvidenceRoot "ramses-rmlui-planet-hub-h2-output.txt"
$readbackPath = Join-Path $EvidenceRoot "ramses-rmlui-planet-hub-h2-readback.bmp"
$summaryPath = Join-Path $EvidenceRoot "ramses-rmlui-planet-hub-h2-summary.json"
...
try {
    Write-Host "SGFX Ramses -> RmlUi planet hub H2"
    ...
    Invoke-CaptureNative "run Ramses -> RmlUi planet hub H2 shell" $outputPath {
        & $shellExe `
            --frames $Frames `
    ...
    $summary = [ordered]@{
        ...
        clean_room = "SGFX-owned procedural faceted sphere and UI nodes; not Earth; no game art/assets"
    }
    $summary | ConvertTo-Json -Depth 4 | Set-Content -LiteralPath $summaryPath -Encoding UTF8
    Write-Host "SGFX planet hub H2 summary: $summaryPath"
    Write-Host "SGFX planet hub H2 evidence complete."
}
finally {
