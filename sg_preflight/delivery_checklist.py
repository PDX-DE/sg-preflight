...
def resolve_delivery_checklist_workbook(
    *,
    ...
    candidates = []
    # When the legacy lookup misses, fall back to the multi-location finder
    ...
    if profile_id:
        try:
            ...
            # openpyxl missing → leave behaviour identical to the legacy single-path lookup.
            pass
...
def delivery_workbook_missing_summary(
    ...
    *,
    ...
    """Honest wording that reports the multi-location workbook-finder search results
    when the finder ran (8 paths checked + raw-data availability), and the legacy
    single-path wording only when the finder hasn't been invoked.
    """
    ...
    # When workspace is available, consult the workbook finder so the operator sees
...
def read_delivery_checklist(
    *,
    ...
    # Activate the workbook finder + generator at the call site. Pre-fix the
