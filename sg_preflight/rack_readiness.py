...
MANUAL_REVIEW_BANNER = (
    "Evidence only - SGFX auto-checks the asset side: exported RCA, delivered CHANGELOG status, "
    "and version metadata. Rack hardware, SVT staging, environment setup, flash outcome, and performance "
...
def _check_delivered(delivery_entry: DeliveryReadinessEntry, cross_entry: CrossDomainItem | None) -> AssetCheck:
    status = cross_entry.delivery_status if cross_entry is not None else delivery_entry.status
    if status == STATUS_DELIVERED:
        date = cross_entry.delivered_date if cross_entry is not None else delivery_entry.delivered_date
        detail = f"Latest CHANGELOG entry is delivered{f' on {date}' if date else ''}."
        return AssetCheck(key="delivered", label="CHANGELOG delivered", passed=True, status="ok", detail=detail)
    return AssetCheck(
        key="delivered",
        label="CHANGELOG delivered",
        passed=False,
        status="blocked",
        detail="Latest CHANGELOG entry is not delivered.",
    )
...
def _entry_from_delivery(
    ...
    *,
    ...
    checks = (
        ...
        _check_delivered(delivery_entry, cross_entry),
...
def rack_readiness_markdown(board: RackReadinessBoard) -> str:
    ...
    lines = [
        ...
        "",
        "| Brand | Model | Asset status | Export | Delivery | Version metadata | Expected SVT | Path |",
        "| --- | --- | --- | --- | --- | --- | --- | --- |",
    ]
        ...
        lines.append(
            ...
            + " | ".join(
                (
                    ...
                    _markdown_cell(checks["delivered"].status),
