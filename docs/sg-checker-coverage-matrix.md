...
- Repo checker and scene check now both feed one normalized checker-evidence payload, so action pages, Files And Proof, stage readiness, and copy exports can reuse the same file-backed evidence surface.
- The remaining maturity gap is no longer basic SG checker awareness or first-pass repo/scene parsing; it is broader checker-output coverage outside those two paths plus BMW-side access/execution.
