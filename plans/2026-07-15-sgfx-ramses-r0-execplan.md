...
- [x] Step 4: Authored-frame/lifecycle lane (`--perspective` + `--perspective-id`): shared render support extracted to `cpp/src/ramses_render_support.h` (preview refactored onto it, behavior-identical); strict perspective parser from the proven cinematic_shell contract; one 480×270 readback driven via `AutoAspect`-only camera-crane semantics (interfaces first, scripts fallback); lifecycle evidence recorded through `drive_probe_lifecycle` (30 s budget, measured); frame classified and `first-frame.png` written only after successful readback (1 MiB cap); explicit `missing_contract` result at exit 0 when the property tree is absent (req 14); real-renderer test proves the full pipeline with a deterministic black frame.
